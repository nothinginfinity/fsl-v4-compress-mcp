...
async function handle(name,args,env,ctx){
  if(name==="afo_fsl_v4_compress_status"){const res={status:"ok",worker:WORKER_NAME,version:VERSION,generated_at:"2026-06-22T15:09:28.647Z",bindings:{},tools:TOOLS.map(t=>t.name)};
  ...
  if (name === "query_v4") {
    ...
    const branch=args.branch||'main',repoKey=owner+'/'+repo,termLower=(term||'').toLowerCase();const limit=Math.min(args.limit||20,50);const rows=await dbAll(env.DB,'SELECT chunk_id,file_path,chunk_type,top_keywords,orig_bytes FROM fsl_v4_chunks WHERE repo_key=? AND branch=? AND (lower(top_keywords) LIKE ? OR lower(file_path) LIKE ?) LIMIT ?',[repoKey,branch,'%'+termLower+'%','%'+termLower+'%',limit]);return{ok:true,repo:repoKey,branch,term,count:rows.length,matches:rows,content_available:false,note:'V4 is lossy - matches show fingerprints only, not source content.'};
  }
  ...
  if (name === "compression_stats_v4") {
    ...
    const branch=args.branch||'main',repoKey=owner+'/'+repo;const rows=await dbAll(env.DB,'SELECT * FROM fsl_v4_jobs WHERE repo_key=? AND branch=? ORDER BY created_at DESC LIMIT 1',[repoKey,branch]);if(!rows.length)return{ok:false,error:'no job found for this repo/branch'};return{ok:true,job:rows[0]};
  }
